...
const createPartialDelivered = async (
  ...
  try {
    ...
    return formatDeliveredPartialReturn(
      estimatedTons,
      tonPerHour,
      estimatedPerGoal,
      realTons,
...
const calcEstimatedPerGoal = (
  workFronts: CttWorkFronts[],
  estimatedTons: CttEstimatedTons
): CttTon => {
    ...
    Object.entries(estimatedTons).forEach(([workFront, ton]) => {
      if (workFront !== "estimated" && typeof ton === "number") {
        if (workFrontGoal.code == +workFront) {
          estimatedPerGoal[+workFront] = normalizeCalc(
            (ton / workFrontGoal.goal) * 100,
            2
          );
        }
      }
...
const formatDeliveredPartialReturn = async (
  estimatedTons: CttEstimatedTons,
  tonPerHour: CttTon,
  estimatedPerGoal: CttTon,
  realTons: CttTon,
  workFronts: CttWorkFronts[]
): Promise<CttPartialDeliveredResult> => {
  const delivered: CttDeliveredReturn[] = [];
  const goalMap = new Map(
    workFronts.map((workFront) => [workFront.code, workFront.goal])
  );
  ...
  for (const key of Object.keys(estimatedTons)) {
    ...
    delivered.push({
      ...
      realTons: normalizeCalc(realTons[key], 2) || 0,
...
const calcEstimatedPercentage = (
  ...
  estimatedTons.estimated.progress = normalizeCalc(
    (estimatedTons.estimated.total / frontGoal) * 100,
    2
  );
  return estimatedTons;
};
