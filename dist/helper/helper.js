...
const getTotalHourmeter = (hourmeters, firstHourmeterValue) => {
    ...
    if (hourmeterWithoutAnomalies.length > 0) {
        ...
        let lastHourmeter = hourmeterWithoutAnomalies.length > 1
            ? Number(hourmeterWithoutAnomalies[hourmeterWithoutAnomalies.length - 1])
            : firstHourmeter;
        if (lastHourmeter <= firstHourmeter &&
            hourmeterWithoutAnomalies.length > 1) {
            lastHourmeter = Number(hourmeterWithoutAnomalies[hourmeterWithoutAnomalies.length - 2]);
        }
        if (lastHourmeter > firstHourmeter) {
