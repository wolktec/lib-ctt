 ...
 */
const createAvailabilityAllocation = async (monitoringCenterAvailability) => {
    ...
    const formattedGroupsData = groups.map((groupData) => {
        ...
        const totalAvailability = formattedWorkFrontsData.reduce((acc, workFront) => acc + (workFront.availability || 0), 0);
        const averageAvailability = totalWorkFronts > 0 ? totalAvailability / totalWorkFronts : 0;
        return {
            group,
    ...
    const cttAvailabilityAndAllocation = {
